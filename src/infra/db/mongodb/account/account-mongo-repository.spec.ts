...
describe('Account Mongo Repository', () => {
  ...
  describe('add()', () => {
    test('Should return an account on add success ', async () => {
      ...
      expect(account.password).toBe('hashed_pass')
    })
  })
  ...
  describe('updateAccessToken()', () => {
    test('Should update the account accessToken on updateAccessToken success', async () => {
      ...
      const account = MongoHelper.map(await accountCollection.findOne({ _id: result.insertedId }))
      expect(account.accessToken).toBeFalsy()
      await sut.updateAccessToken(account.id, 'any_token')
      const accountAfterUpdate = MongoHelper.map(await accountCollection.findOne({ _id: result.insertedId }))
